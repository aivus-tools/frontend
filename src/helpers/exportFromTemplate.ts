...
/** Возвращает удобную ссылку на именованную ячейку */
function getDefinedRef(wb: ExcelJS.Workbook, name: string): DefinedRef {
  ...
  if (!sheet) throw new Error(`Worksheet "${sheetName}" not found for named range "${name}"`);
  const rc = a1ToRC(addr);
  return { sheet, a1: addr, row: rc.row, col: rc.col };
}

/** Устанавливает значение в именованную ячейку */
...
export async function exportFromTemplate(data: DataRow[]) {
  // 1) шаблон из /public
  const res = await fetch('/template.xlsx');
  if (!res.ok) throw new Error(`Failed to fetch template.xlsx: ${res.status} ${res.statusText}`);
  ...
  // 3) подставляем именованные
  setNamedCell(wb, 'CompanyName', 'ООО «Пример»');
  setNamedCell(wb, 'ReportDate', new Date()); // формат даты задайте в шаблоне

  // 4) вставляем строки начиная с TableStart
