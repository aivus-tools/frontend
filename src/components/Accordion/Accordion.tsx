...
export const Accordion = ({ title, children, className, onAccToggle, ...props }: AccordionProps) => {
	...
	return (
		<div className={cn(styles.accordion, className)} {...props}>
			<div className={cn(styles.title, { [styles.opened]: isOpen })} onClick={toggleAccordion}>
				<ArrowIcon className={cn(styles.icon)} />
				{title}
			</div>
			<div ref={contentRef} style={{ height: height }} className={cn(styles.content, { [styles.opened]: isOpen })}>
