...
export const Uploader: React.FC<UploaderProps> = ({ onChange, thumbnailUrl }) => {
  ...
  React.useEffect(() => {
    if (!initialized && thumbnailUrl && fileList.length === 0) {
      setFileList([
        {
          uid: 'existing',
          name: 'thumbnail',
  ...
  }, [thumbnailUrl, initialized, fileList.length]);
  ...
  const handleChange: UploadProps['onChange'] = ({ fileList: newFileList }) => {
    ...
    if (lastFile?.originFileObj) {
      onChange?.(lastFile.originFileObj);
