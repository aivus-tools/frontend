...
export const Agency = () => {
  ...
  return (
    <>
      {modal}
      <Form.Item name='agencyName' label={t('AGENCY_NAME')} extra={t('AGENCY_ASSOCIATED')} style={{ width: '100%' }}>
      ...
      <Form.Item label={<LabelWithAdd text={t('AGENCY_PRODUCERS')} onClick={() => showModal()} />}>
        {agencyProducers.length === 0 && <Typography.Text type='secondary'>{t('EMPTY')}</Typography.Text>}
        {agencyProducers.map((x) => {
          ...
          return (
            <Flex gap={12} key={x.originalIndex} align='center' style={{ marginBottom: 8 }}>
              ...
              <div className={commonStyles.iconButton} onClick={() => removeAgencyProducer(x.originalIndex)}>
