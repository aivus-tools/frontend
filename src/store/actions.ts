...
import { TSection, TRow } from '@/interfaces/app.interface';
...
export const updateItem = createAction<{ sectionId: number; rowId: number; field: string; value: string | number }>(
	'estimates/updateItem'
);
export const updateSectionVisibility = createAction<{ sectionId: number; isHidden: boolean }>(
	'estimates/updateSectionVisibility'
);
