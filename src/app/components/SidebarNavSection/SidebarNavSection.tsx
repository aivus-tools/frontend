...
import ArrowIcon from '@/app/icons/arrow-left-icon.svg';
...
import { updateSectionVisibility } from '@/app/store/actions';
	...
	return (
		<div className={cn(styles.section, className)} {...props}>
			<div className={cn(styles.item, { [styles.row]: section.type === 'row' })} onClick={toggleSection}>
				{section.type !== 'row' && (
					<ArrowIcon className={cn(styles.icon, { [styles.rotated]: isOpen })} />
				)}
